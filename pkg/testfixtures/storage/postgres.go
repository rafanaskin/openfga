...
func RunPostgresTestContainer(t testing.TB) DatastoreTestContainer {
	...
	require.NoError(t, waitForDatabase("pgx", tplURI))
	...
	require.NoError(t, waitForDatabase("pgx", testCont.GetConnectionURI(true)))
	...
	db, err := goose.OpenDBWithDriver("pgx", testCont.GetConnectionURI(true))
	require.NoError(t, err)
	defer db.Close()
